...
export default function CampaignDetailClient({ campaign, featuredImage }) {
    ...
    const additionalInfo = campaign.acf?.additional_info || null;

    // ACF detail fields (non-translated — dates/locations stay in Thai)
    const hasDetails = campaign.acf && !Array.isArray(campaign.acf) && (
        campaign.acf.campaign_goal || campaign.acf.start_date ||
        campaign.acf.location || campaign.acf.status
    );

    const campaignGoal = lang === "en" && campaign.acf?.campaign_goal_en
        ? campaign.acf.campaign_goal_en
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {hasDetails && (
                        <div className="bg-brand-white/20 border border-brand-white rounded-[24px] p-8 md:p-10 mb-16 shadow-sm relative overflow-hidden">
                            ...
                            <h2 className="text-2xl md:text-3xl font-bold text-brand-green-dark mb-8 border-b border-brand-green-dark/20 pb-4 relative z-10">
                                {lang === "en" ? "Campaign Details" : "รายละเอียดการดำเนินการ"}
                            </h2>
                                ...
                                {campaignGoal && (
                                    <div>
                                        <h3 className="text-xs font-bold text-brand-green-dark uppercase tracking-wider mb-2 flex items-center gap-2">
                                            <span className="w-2 h-2 rounded-full bg-brand-white"></span>
                                            {lang === "en" ? "Goal" : "เป้าหมาย"}
                                        </h3>
                    ...
                    <div className="mt-20 pt-10 border-t border-gray-100 flex justify-center text-center">
                        ...
                        >
                            ...
                            {lang === "en" ? "Back to all campaigns" : "กลับไปยังหน้างานรณรงค์ทั้งหมด"}
                        </a>
                    </div>
                </div>
            </article>
